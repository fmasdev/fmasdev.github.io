<template>
  <div class="mb-12 p-6 rounded-2xl shadow-md bg-white border border-gray-200">
    ...
    >
      <div>
        <TitleComponent :title="feedBack.title" level="h3" />
        <p class="text-sm text-gray-600">
          {{ dateHelper.dateStringFormatToString(feedBack.date) }}
        </p>
      </div>
      <div>
        ...
        <p
          class="py-0.5 rounded-full text-xs font-medium bg-primaryLight text-gray-600"
        >
          {{ feedBack.job }}
        </p>
      </div>
    ...
    <TextMarkdownComponent :text="feedBack.content" />
  </div>
</template>
